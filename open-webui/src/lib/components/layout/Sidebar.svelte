...
{#if $showSidebar}
	...
	>
		...
		>
			...
			<div
				class="relative flex flex-col flex-1 scrollbar-hidden pt-3 pb-3"
				on:scroll={(e) => {
...
<style>
    /* 1. 사이드바 내부에서 튀어나오는 모든 요소 허용 */
    :global(.overflow-y-auto) {
        overflow: visible !important;
    }

    /* 2. 드롭다운 메뉴 스타일 강제 고정 */
    :global([data-melt-dropdown-menu]) {
        background-color: white !important; /* 배경색이 투명해지지 않도록 */
        border: 1px solid #e5e5e5 !important; /* 테두리 추가 */
        border-radius: 8px !important;
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1) !important; /* 그림자 추가 */
        z-index: 9999 !important; /* 최상단 레이어 */
        min-width: 200px !important;
        padding: 8px !important;
        color: black !important; /* 글자색 강제 */
    }

    /* 3. 내부 아이템들 정렬 */
    :global([data-melt-dropdown-menu] [data-melt-dropdown-menu-item]) {
