			...
			<div class=" text-lg font-medium self-center flex items-center">
				{#if citation?.source?.name}
					...
					{#if document?.metadata?.file_id || document.source?.url?.includes('http')}
						...
						>
							{@const page = document?.metadata?.page ?? 0}
							{@const baseUrl = document?.source?.url?.split('#')[0]}

							<a
								class="hover:text-gray-500 dark:hover:text-gray-100 underline grow line-clamp-1"
								href={document?.metadata?.file_id
									? `${WEBUI_API_BASE_URL}/files/${document?.metadata?.file_id}/content#page=${page + 1}`
						...
						<div>
							...
							>
								{#if document.source?.url?.includes('http')}
									{@const baseUrl = document.source.url?.split('#')[0]}
									{@const hasPage = Number.isInteger(document?.metadata?.page)}
									{@const snippetUrl = hasPage
										? `${baseUrl}#page=${document.metadata.page + 1}`
										: baseUrl}
								...
								{#if Number.isInteger(document?.metadata?.page)}
									<span class="text-sm text-gray-500 dark:text-gray-400">
										(PDF p.{document.metadata.page + 1})
									</span>
								{/if}
							</div>
