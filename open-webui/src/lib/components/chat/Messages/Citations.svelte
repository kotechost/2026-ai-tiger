...
{#if citations.length > 0}
	...
	<div class=" py-1 -mx-0.5 w-full flex gap-1 items-center flex-wrap">
		...
		>
			...
			{/if}
			<div>
				📄 문서 근거 {citations.length > 0 ? `(${citations.length})` : ''}
			</div>
		</button>
	</div>
{/if}
			...
			{#each citations as citation, idx}
				<button
					...
					aria-label={$i18n.t('View source: {{name}}', {
						name: decodeString(citation.source?.name || '문서')
					})}
				...
				>
					...
					<div class="flex flex-col text-left flex-1">
						<div class="truncate hover:text-black dark:hover:text-white transition font-medium">
							📄 {decodeString(citation.source?.name || '문서').replaceAll('_', ' ')}
						</div>

						<div class="text-[11px] text-gray-400 mt-0.5">
							근거 {citation.metadata?.length}개
						</div>
					</div>
				</button>
			{/each}
