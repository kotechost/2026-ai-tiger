<script lang="ts">
	...
	const chatEventHandler = async (event, cb) => {
		...
		if (event.chat_id === $chatId) {
			...
			if (message) {
				...
				console.log('socket chat_id', event.chat_id, 'current chat_id', $chatId, 'type', type, data);
	...
	const initNewChat = async () => {
		...
		// 1. 세션 꼬임 방지 핵심: 모든 사용자에게 임시 채팅 모드 강제 활성화 (AUTH=False 대응)
		...
		// URL 파라미터 처리 (Youtube, Web Search 등)
